...
<table id="zero_config" class="table search-table align-middle text-nowrap dataTable">
  ...
  <tbody>
  	<?php
  		$invoices = $con->getRows('invoices', array('where'=>'invoice_status=0 and sacco_id="'.$_SESSION['USR_OF'].'"', 'order_by'=>'date_posted desc'));
  		if(!empty($invoices)){
  			$i=0;
...
<table id="zero_config" class="table search-table align-middle text-nowrap dataTable">
  ...
  <tbody>
  	<?php
  		$invoices = $con->getRows('invoices', array('where'=>'invoice_status=1 and sacco_id="'.$_SESSION['USR_OF'].'"', 'order_by'=>'date_posted desc'));
  		if(!empty($invoices)){
  			$i=0;
  			foreach($invoices as $invoice){ 
  				$i++;
