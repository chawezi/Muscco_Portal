  ...
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    <li class="nav-item">
      <button class="
        ...
        <span class="d-none d-md-block font-weight-medium">All Pending and Paid Invoices</span>
      </button>
    </li>
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_all" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      ...
      $paid = 0;
      $amount_paid = 0;
      $all = $con->getRows('invoices', array('where'=>'invoice_status !=2'));
      if(!empty($all)){        
        foreach($all as $count){
          $updates = $con->getRows('invoice_status', array('where'=>'invoice_id="'.$count['invoice_id'].'"'));
          if(!empty($updates)){
            foreach ($updates as $key) {
              $amount_paid += $key['paid_amount']; 
            }              
          }  
          $amounts +=$count['amount'];
          $paid = $count['amount_paid'] + $amount_paid;
        }

        $balances = $amounts - $paid;
        
      }
    ?>
    <th>Total Invoice Amount<br> MK<?=number_format($amounts,2,'.',',')?></th>
    <th>Amount Paid<br> MK<?=number_format($paid,2,'.',',')?></th>
...
<table id="scroll_hor" class="table border table-striped table-bordered display dataTable no-footer" style="width: 100%;" aria-describedby="scroll_hor_info">
  ...
  <tbody>
    <?php
      $invoices = $con->getRows('invoices a, sacco b', array('where'=>'a.invoice_status !=2 and a.sacco_id=b.sacco_id', 'order_by'=>'date_posted desc'));
      if(!empty($invoices)){
        $balance = 0;
        $paid = 0;
        $amount_paid = 0;
        foreach($invoices as $invoice){ 
          $updates = $con->getRows('invoice_status', array('where'=>'invoice_id="'.$invoice['invoice_id'].'"','return_type'=>'single'));
          if(!empty($updates)){
              $amount_paid = $updates['paid_amount']; 
            }          
          $balance = $invoice['amount'] - ($invoice['amount_paid'] + $amount_paid);
          $paid = $invoice['amount_paid'] + $amount_paid;
    ?>
...
<?php }else if($action == "invoice_today"){ ?>
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    <li class="nav-item">
      <button class="
            ...
            " id="all-category">
        <i class="ti ti-report fill-white me-0 me-md-1"></i>
        <span class="d-none d-md-block font-weight-medium"> Invoices Posted Today <?=date("d M, Y");?></span>
      </button>
    </li>
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_today" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
       <i class="ti ti-download me-0 me-md-1 fs-4"></i>
        <span class="d-none d-md-block font-weight-medium fs-3">Download</span>
      </a>
    </li>
...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      ...
      $amount_paid = 0;
      $date = date("Y-m-d");
      $all = $con->getRows('invoices', array('where'=>'invoice_status !=2 and date_posted="'.$date.'"'));
      if(!empty($all)){        
        foreach($all as $count){
          $updates = $con->getRows('invoice_status', array('where'=>'invoice_id="'.$count['invoice_id'].'"'));
          if(!empty($updates)){
...
<table id="scroll_hor" class="table border table-striped table-bordered display dataTable no-footer" style="width: 100%;" aria-describedby="scroll_hor_info">
  ...
  <tbody>
    <?php
      $invoices = $con->getRows('invoices a, sacco b', array('where'=>'a.invoice_status !=2 and a.sacco_id=b.sacco_id and a.date_posted="'.$date.'"', 'order_by'=>'date_posted desc'));
      if(!empty($invoices)){
        $balance = 0;
        $paid = 0;
        $amount_paid = 0;
...
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    ...
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_week" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
       <i class="ti ti-download me-0 me-md-1 fs-4"></i>
        <span class="d-none d-md-block font-weight-medium fs-3">Download</span>
      </a>
    </li>
...
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    ...
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_month" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
       <i class="ti ti-download me-0 me-md-1 fs-4"></i>
        <span class="d-none d-md-block font-weight-medium fs-3">Download</span>
      </a>
    </li>
...
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    ...
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_year" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
       <i class="ti ti-download me-0 me-md-1 fs-4"></i>
        <span class="d-none d-md-block font-weight-medium fs-3">Download</span>
      </a>
    </li>
...
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
    ...
    <li class="nav-item ms-auto">
      <a href="../../generate-lpdf.php?type=invoice_request_custom&date_from=<?=$first_date?>&date_to=<?=$last_date?>&officer=<?=$section?>" target="_blank" class="btn btn-primary d-flex align-items-center px-3" id="add-notes">
       <i class="ti ti-download me-0 me-md-1 fs-4"></i>
        <span class="d-none d-md-block font-weight-medium fs-3">Download</span>
      </a>
    </li>
