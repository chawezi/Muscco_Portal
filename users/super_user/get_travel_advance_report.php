...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2'));
      if(!empty($all)){
        $allowances = 0;
        $fuels = 0;
        $total = 0;
    ...
    <tbody>
      <?php
        $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2', 'order_by'=>'a.date_posted desc'));
        if(!empty($leave)){
          $i=0;
...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      ...
      $fuels = 0;
      $total = 0;
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2 and date_approved="'.$date.'"'));
      if(!empty($all)){
...
<table id="scroll_hor" class="table border table-striped table-bordered display dataTable no-footer" style="width: 100%;" aria-describedby="scroll_hor_info">
    ...
    <tbody>
      <?php
        ...
        $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2 and a.date_approved="'.$date.'"', 'order_by'=>'a.date_posted desc'));
        if(!empty($leave)){
          $i=0;
          foreach($leave as $day){ 
            $i++;
...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      ...
      $fuels = 0;
      $total = 0;
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2 and date_approved >="'.$monday.'" and date_approved <= "'.$saturday.'"'));
      if(!empty($all)){
...
<table id="scroll_hor" class="table border table-striped table-bordered display dataTable no-footer" style="width: 100%;" aria-describedby="scroll_hor_info">
    ...
    <tbody>
      <?php
        $date = date("Y-m-d");
        $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.employee_id="'.$_SESSION['USR_ID'].'" and a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2 and a.date_approved >="'.$monday.'" and a.date_approved <= "'.$saturday.'"', 'order_by'=>'a.date_posted desc'));
        if(!empty($leave)){
          $i=0;
          foreach($leave as $day){ 
            $i++;
...
<table class="table border table-striped table-bordered">
  <tr>
    <?php
      ...
      $fuels = 0;
      $total = 0;
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2 and date_approved >="'.$first_date.'" and date_approved <= "'.$last_date.'"'));
      if(!empty($all)){
...
<table id="scroll_hor" class="table border table-striped table-bordered display dataTable no-footer" style="width: 100%;" aria-describedby="scroll_hor_info">
    ...
    <tbody>
      <?php
        $date = date("Y-m-d");
        $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.employee_id="'.$_SESSION['USR_ID'].'" and a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2 and a.date_approved >="'.$first_date.'" and a.date_approved <= "'.$last_date.'"', 'order_by'=>'a.date_posted desc'));
        if(!empty($leave)){
          $i=0;
          foreach($leave as $day){ 
            $i++;
...
<?php 
    ...
    if($officer == 'All'){
      ...
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2 and date_approved >="'.$first_date.'" and date_approved <= "'.$last_date.'"'));
      $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2 and a.date_approved >="'.$first_date.'" and a.date_approved <= "'.$last_date.'"', 'order_by'=>'a.date_posted desc'));
    }else{
      ...
      $all = $con->getRows('travel_advance_request', array('where'=>'request_status=2 and date_approved >="'.$first_date.'" and date_approved <= "'.$last_date.'" and employee_id="'.$officer.'"'));
      $leave = $con->getRows('travel_advance_request a, pillars b, muscco_members c, departments d',
                         array('where'=>'a.pillar=b.pillar_id and a.employee_id=c.muscco_member_id and c.department_id=d.department_id and a.request_status=2 and a.date_approved >="'.$first_date.'" and a.date_approved <= "'.$last_date.'"  and a.employee_id="'.$officer.'"', 'order_by'=>'a.date_approved desc'));
    }    
  ?>
<div class="container-fluid note-has-grid" style="width:100%; padding:2px;">
  <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
